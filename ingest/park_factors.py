...
def compute_k_park_factors(start, end=None):
    ...
    k_by_park = (
        ...
        .agg({"is_k": "sum", "batter": "count"})  # rough PA proxy
        .reset_index()
    )
    ...
    k_by_park["Team_abbr"] = (
        k_by_park["home_team"].map(team_fix_map).fillna(k_by_park["home_team"])
    )

    k_by_park[["Team_abbr", "K_park_factor"]].to_csv(
        "../data/raw/park_factors_2025.csv"
    )
