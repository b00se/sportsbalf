...
def get_ud_strikeouts_json():
    ...
    params = {"sport_id": "HOME", "algolia_object_id": algolia_object_id}
...
def parse_strikeout_lines(data: dict) -> list[dict]:
    ...
    starter_ids = {
        ...
        if any(
            badge.get("label") == "Starting" and badge.get("value") == "Yes"
    ...
    for line in data.get("over_under_lines", []):
        appearance_id = (
            line.get("over_under", {}).get("appearance_stat", {}).get("appearance_id")
        )
...
def main():
    ...
    for line in lines:
        over_fmt = (
            f"{line['over_american_price']}/{line['over_decimal_price']}"
            f"x{line['over_payout_multiplier']}"
        )
        under_fmt = (
            f"{line['under_american_price']}/{line['under_decimal_price']}"
            f"x{line['under_payout_multiplier']}"
        )
        print(f"{line['player']:20} {line['k_line']:>4}     {over_fmt:20} {under_fmt}")
