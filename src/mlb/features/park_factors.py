"""Park factor utilities for MLB pitching models."""
...
def load_live_park_factors(csv_path: str) -> pd.DataFrame:
    """Load latest park factor aggregates from a CSV with the expected columns."""
    df = pd.read_csv(csv_path)
    return df[["Team_abbr", "K_park_factor"]]


def compute_k_park_factors(start_date, end_date, source_df=None):
    """Compute park strikeout factors over the provided Statcast window."""
    ...
    k_by_park["Team_abbr"] = (
        k_by_park["home_team"].replace(team_fix_map).fillna(k_by_park["home_team"])
    )
